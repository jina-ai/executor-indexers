...
def assert_dump_data(dump_path, docs, shards, pea_id):
    ...
    np.testing.assert_equal(
        metas_dump,
        [
            FileIndexer._doc_without_embedding(d).SerializeToString()
...
def test_dbms_file_writer(tmpdir):
    ...
    with FileIndexer(index_filename='dbms', metas=metas) as indexer:
        indexer.add(docs)
    ...
    with FileIndexer(dump_on_exit=True, index_filename='dbms', metas=metas) as indexer:
...
def test_filedbms_crud(tmpdir, nr_docs=10):
    ...
    # indexing
    indexer = FileIndexer(dump_on_exit=False, metas=metas)
    indexer.add(docs)
    assert indexer.size == len(docs)
